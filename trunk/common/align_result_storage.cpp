...
bool AlignResultStorage :: add_result (longlong query_id, longlong search_id, bool reverse, float al_score, int score, float chi2, double evalue, double bitscore, int q_auto_score, int t_auto_score, int batch_no, BATCH* batches)
{
    ...
    else
    {
        ...
        {
            AlignResult d (search_id, reverse, al_score, score, chi2, evalue, bitscore, 0, 0, 0, NULL); // no batch array allocation / copying here
            if (d < q->top ())
    ...
    AlignResult r (search_id, reverse, al_score, score, chi2, evalue, bitscore, q_auto_score, t_auto_score, batch_no, batches);
