...
double *get_h(WMatrixType &matrix, ResFreqType &freq1, ResFreqType &freq2, int *hmin, int *hmax, double *mean);
...
int
KarlinAltschulStatistics(WMatrixType &matrix, ResFreqType &freq1, ResFreqType &freq2, double *lambda, double *Kminus, double *Kplus, double *H, double *r, double *s ) {
...
double *get_h(WMatrixType &matrix, ResFreqType &freq1, ResFreqType &freq2, int *hmin, int *hmax, double *mean) {
  ...
  *mean = 0.0;
  ...
  for (n = 0; n < AANUM; n++)
    for(m = 0; m < AANUM; m++) {
      if (WMatrixXY(matrix, n, m) > *hmax)
        *hmax = WMatrixXY(matrix, n, m);
      if (WMatrixXY(matrix, n, m) < *hmin)
        *hmin = WMatrixXY(matrix, n, m);
    }
  ...
  for (n = 0; n < AANUM; n++)
    for(m = 0; m < AANUM; m++)
      distribution[WMatrixXY(matrix, n, m)] += freq1[n]*freq2[m];
